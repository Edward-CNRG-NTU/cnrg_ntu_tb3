...
def visualizer():
    ...
    def lso_cb(data):
        t1 = time.time()
        data_np = np.clip(np.min(np.array(data.data).reshape(data.shape), axis=1, keepdims=True), 0, 1)
    ...
    rospy.Subscriber(SUB_TOPIC_NAME, AuditoryNerveImageMultiDim, lso_cb)
